...
function moveInvaders() { ///moves the enemy invaders
  ...
  if (squares[currentShooterIndex].classList.contains('invader', 'shooter')) { ///checks to see if the invader and the player shooter share the same square, if so do: x 
    ...
    $("#background-video").attr("src","Markiplier there will be bloodshed.mp4"); //video that plays when game over
...
function slug2(e) { ///makes the shooter fire a laser 
  ...
  function moveLaser() { ///gives the laser motion
    ...
    if (squares[currentLeft].classList.contains('invader')) { ///if the laser shares the same square as an invader|basically a collision function
...
///https://www.youtube.com/watch?v=UPXULv7qRw4  
///https://www.youtube.com/watch?v=yJH3jCY2nzA  
///https://www.youtube.com/watch?v=vKmmIShTOAA 
///https://www.youtube.com/watch?v=_i3nvGRNw-o
///https://www.youtube.com/watch?v=7zYjL6ZpXBw 
...
/*function Crucible(e){ 
  ...
  function moveLaser() { ///gives the laser motion
    squares[currentFront].classList.remove('laser') ///removes the laser from where it is currently to later draw it in a later location
    ...
    if (squares[currentFront].classList.contains('invader')) { ///if the laser shares the same square as an invader|basically a collision function
      ...
      const alienRemoved = alienInvaders.indexOf(currentFront) ///passes through the laser index in order to have the square the alien was in to be removed from the array
      aliensRemoved.push(alienRemoved) ///executes the code of alienRemoved 
      results++ ///increases by 1 everytime an invader has been hit
      resultsDisplay.innerHTML = results ///shows the score on screen via the html
      console.log(aliensRemoved) ///shows in the console what invader has been hit via its number value, the lists goes from 1st to last since the last invader has been hit
...
document.addEventListener('keyup', Crucible) 
*/
////hard mode////
///the enemies take 2 shots to kill/// 
///slower shots/// 
